...
function validateNumber(evt) {
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Billing Entry</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
                ...
                <div class="col-lg-12">
						 ...
						 <form role="form" method="post">
                            <div class="row">
                                <div class="col-lg-6">
                                        ...
                                        <div class="form-group">
                                            <label>Date*:</label>
                                            <input class="form-control" value="<?php echo date('Y-m-d'); ?>" type="date" name="entry_data" id="date" required>
                                        </div>
                                       <div class="form-group">
                    ...
                    <div class="panel panel-default">
						...
						if(isset($_POST['submit'])){
							...
							$sql = "INSERT INTO sale (customer_id,tot_cash_amt,net_cash_amt,tax,discount,payment_amount,remark,entry_data) Values('$customer_id','$tot_cash_amt','$net_cash_amt','$tax','$discount','$payment_amount','$remark','$newDate')";
							$dbh->exec($sql);
							...
							$sqlp = "INSERT INTO sale_payment(sale_id,payment_amount,remark,entry_data,payment_mode,cheque_no,cheque_date,bank_name,narration) VALUES('$sale_id','$payment_amount','$remark','$newDate','$payment_mode','$cheque_no','$cheque_date','$bank_name','$narration')";
							$dbh->exec($sqlp);
							}
							//echo "<br>";
							...
							while($i<count($product_id)){
								...
								$sqlitem = mysqli_query($con,"SELECT quantity,myprice FROM product WHERE product_id=$product_id[$i]");
								$resitem = mysqli_fetch_array($sqlitem);
								$oldquantity = $resitem['quantity'];
								$myprice = $resitem['myprice'];
								$quantity[$i];
								...
								$sql = "INSERT INTO sale_product (sale_id,category_id,product_id,quantity,price,myprice) Values('$sale_id','$category_id[$i]','$product_id[$i]','$quantity[$i]','$rate[$i]','$myprice')";
								$dbh->exec($sql);
			...
			$sql="INSERT INTO transaction(date,amount,narration,payment_mode,fee_id,sale_id) VALUES('$newDate','$payment_amount','Sale Payment','$payment_mode','$sale_id','$sale_id')";
			$dbh->exec($sql);
